	...
	if (document.images[i].width>550)
	 {
       ...
       try{
	       document.images[i].outerHTML='<a href="'+document.images[i].src+'" target="_blank" title="رەسىمنى يېڭى كۆزنەكتە ئېچىش">'+document.images[i].outerHTML+'</a>'
  	 	}catch(e){}
  	 }
   }
  else{
	if (document.images[i].width>400) {
	  document.images[i].title="رەسىمنى يېڭى كۆزنەكتە ئېچىش"
...
function doCopy(ID) { 
	if (document.all){
		 ...
		 alert("كود كېسىپ چاپلاش تاختىسىغا كۆچۈرۈلدى");
	}
	else{
		 alert("بۇ ئىقتىدار پەقەت IE دىلا ئۈنۈملۈك\n\nتېكىست رايونىدا Ctrl+A نى بېسىپ تۇرۇپ قايتا كۆچۈرۈڭ")
	}
}
//另存代码
...
function MediaShow(strType,strID,strURL,intWidth,intHeight)
{
	...
	if(MediaTemp[strID]){
		...
		document.getElementById(strID+"_text").innerHTML="مۇشۇ يەردە قويۇش"	
	}else{
		document.images[strID+"_img"].src="images/mm_snd_stop.gif" 		
		document.getElementById(strID+"_text").innerHTML="كۆرۈشنى توختىتىش"
		switch(strType){
			case "swf":
...
function CheckPost(){
  try{
	if (Trim(document.forms[0].title.value)=="") {
		 alert("يازما تېمىسىنى چوقۇم تولدۇرۇڭ!")
  ...
  try{
    if (document.getElementById("P2").checked) {
    	if (!CheckDate(document.forms[0].PubTime.value)){
    	   alert("ۋاقىت فورماتى توغرا ئەمەس\nئۆلچەملىك شەكلى:yyyy-mm-dd hh:mm:ss")	
		   return false
    	}
    	else
    	{
	...
	type=type.toLowerCase()
 	 switch(type){
 	  ...
 	  default :
        ...
        if (memberDown==1)
	         {parent.document.forms[0].Message.value+='[mDown='+path+']بۇ يەرنى بېسىپ چۈشۈرىۋېلىڭ[/mDown]\n'}
         else
	         {parent.document.forms[0].Message.value+='[down='+path+']بۇ يەرنى بېسىپ چۈشۈرىۋېلىڭ[/down]\n'}
        }
        else{oEditor.InsertHtml('<a href="'+path+'"><img border="0" src="'+Fhref+'images/download.gif" alt="" style="margin:0px 2px -4px 0px"/>بۇ يەرنى بېسىپ چۈشۈرىۋېلىڭ</a>')}
        break;
     }
}
//写入顶部Flash文件
...
function getTrackbackURL(id){
	var strHTML = "";
	strHTML = '<span id="tbSpan">تەستىق نومۇرنى كىرگۈزۈڭ <input id="vCode" maxLength="4" size="4" style="border:1px solid #999;"/> <img id="tbCode" src="about:blank" onerror="src=\'common/getCode.asp?s='+Math.random()+'\'" onclick="src=\'common/getCode.asp?s='+Math.random()+'\'" style="margin-bottom:-4px;height:20px;width:40px;cursor:pointer" title="چەكسىڭىز يېڭىلىنىدۇ" alt="ئوقۇلىۋاتىدۇ..."/> <input type="button" value="ئېرىشىش" onclick="getTB('+id+')"/></span><input id="getTBURL" style="border:1px solid #999;width:100%;display:none">';
	showPopup("نەقىل ئادرېسىغا ئېرىشىش",strHTML,250,200);
}

function getTB(id){
	if (document.getElementById("vCode").value.length <4){
		alert("تەستىق نومۇرنى كىرگۈزۈڭ");
...
function showPopup(title,html,width){
	...
	var strHTML = '<h4 style="color:#000;font-size:14px;margin:4px;margin-bottom:2px">'+ closeButton + (title?title:'تېمىسىز كۆزنەك') + '</h4>';
...
function delCommentConfirm(){
	if (!window.confirm('بۇ ئىنكاسنى ئۆچۈرەمسىز؟')) {return false}
	return true;
}
...
function loadComment(page,needTips){
	...
	if (needTips) {
		fillComment("<div style='margin-bottom:10px;'>ئىنكاس ئۇچۇرلىرى ئوقۇلىۋاتىدۇ...</div>");
	}
...
function replyMsg(logId,id,a1,a2,a3){
	...
	if (!_r) {
		...
		_r.innerHTML = '<br/><div class="UBBPanel replayPanel"><div class="UBBTitle"><img alt="ئىنكاسقا جاۋاب قايتۇرۇش" style="margin: 0px 2px -3px 0px;" src="images/icon_reply.gif"/> ئىنكاسقا جاۋاپ قايتۇرۇش</div><div class="UBBContent">' +
				...
				'<textarea onkeypress="Harp();" name="replay" style="width: 99%; height: 60px;" class="editTextarea" id="edit_'+id+'"></textarea>' +
				'<input type="submit" value="يوللاش" class="userbutton"  id="button_'+id+'"/> <input onclick="removeReplyMsg('+id+')" type="button" value="بولدىلا" class="userbutton"/>' +
